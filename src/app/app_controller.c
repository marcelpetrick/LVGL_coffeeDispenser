...
static void transition(coffee_app_controller_t *app, coffee_app_state_t next, const char *message)
{
...
{
    if (app->state != COFFEE_APP_READY && app->state != COFFEE_APP_BEVERAGE_SELECTED)
        return false;
...
bool coffee_app_controller_confirm(coffee_app_controller_t *app)
{
    ...
    if (!app->service->start(app->service, &app->request, &error)) {
        transition(app, COFFEE_APP_ERROR, error != NULL ? error : "Backend rejected request");
        return false;
    }
...
bool coffee_app_controller_cancel(coffee_app_controller_t *app)
{
    ...
    if (app->state == COFFEE_APP_DISPENSING && app->service->cancel(app->service)) {
        app->dispense = app->service->dispense_snapshot(app->service);
