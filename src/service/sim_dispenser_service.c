...
static coffee_machine_snapshot_t
sim_machine_snapshot(coffee_dispenser_service_t *service) {
  return as_sim(service)->machine;
}

static bool resource_available(const coffee_machine_snapshot_t *machine,
                               uint32_t resource) {
...
static bool sim_can_dispense(coffee_dispenser_service_t *service,
                             const coffee_beverage_t *beverage,
                             const char **reason) {
  coffee_sim_dispenser_t *sim = as_sim(service);
  if (beverage == NULL) {
    if (reason != NULL)
...
static bool sim_start(coffee_dispenser_service_t *service,
                      const coffee_beverage_request_t *request,
                      const char **error) {
  coffee_sim_dispenser_t *sim = as_sim(service);
  ...
  COFFEE_LOGI("dispense accepted: %s %u ml", beverage->stable_id,
              request->volume_ml);
  return true;
}
...
static coffee_dispense_snapshot_t
sim_dispense_snapshot(coffee_dispenser_service_t *service) {
...
static void sim_tick(coffee_dispenser_service_t *service, uint32_t elapsed_ms) {
  ...
  uint32_t remaining_ms = (sim->duration_ms > sim->elapsed_ms)
                              ? (sim->duration_ms - sim->elapsed_ms)
                              : 0;
