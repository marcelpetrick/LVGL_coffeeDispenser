...
static void fail_cb(lv_event_t *e) { LV_UNUSED(e); }
...
void coffee_screen_show_diagnostics(coffee_ui_manager_t *ui) {
  ...
  lv_label_set_text_fmt(info,
                        ...
                        COFFEE_APP_VERSION, LVGL_VERSION_MAJOR,
                        LVGL_VERSION_MINOR, LVGL_VERSION_PATCH,
                        coffee_machine_status_label(machine.status),
                        machine.water_available ? "ok" : "empty",
                        machine.beans_available ? "ok" : "empty",
                        machine.milk_available ? "ok" : "unavailable",
