...
void coffee_screen_show_beverage(coffee_ui_manager_t *ui) {
  ...
  lv_label_set_text_fmt(title, "%s %s", beverage->symbol,
                        beverage->display_name);
  lv_obj_set_style_text_font(title, &lv_font_montserrat_32, 0);
  lv_obj_set_style_text_color(title, coffee_ui_color_text(), 0);

  lv_obj_t *info = lv_label_create(panel);
  lv_label_set_text_fmt(info, "%u ml  |  about %u s  |  %s",
                        ui->app->request.volume_ml,
  ...
  lv_obj_set_style_text_color(note,
                              beverage->id == COFFEE_BEV_HOT_WATER
                                  ? coffee_ui_color_warning()
