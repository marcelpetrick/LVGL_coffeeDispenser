...
static void select_cb(lv_event_t *e)
{
    ...
    if (beverage != NULL && beverage->option_count > 0) {
        coffee_app_controller_select(ui->app, id, beverage->options[beverage->option_count / 2]);
    }
}
...
void coffee_screen_show_home(coffee_ui_manager_t *ui)
{
    ...
    lv_label_set_text_fmt(status, "Status: %s", coffee_machine_status_label(machine.status));
