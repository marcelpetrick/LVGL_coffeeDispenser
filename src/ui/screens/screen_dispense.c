...
void coffee_screen_update_dispense(coffee_ui_manager_t *ui) {
  ...
  if (ui->step_label != NULL)
    lv_label_set_text(ui->step_label, snap.current_step != NULL
                                          ? snap.current_step
                                          : "Working");
  if (ui->progress_label != NULL) {
    lv_label_set_text_fmt(ui->progress_label, "%u%%  |  %u s remaining",
                          snap.progress_pct, snap.remaining_s);
  }
}
...
void coffee_screen_show_completion(coffee_ui_manager_t *ui, bool cancelled) {
  ...
  lv_obj_set_style_text_color(
      title, cancelled ? coffee_ui_color_warning() : coffee_ui_color_primary(),
      0);

  lv_obj_t *message = lv_label_create(screen);
  lv_label_set_text_fmt(
      message, cancelled ? "The cycle stopped safely." : "%s is complete.",
