...
void coffee_ui_manager_init(coffee_ui_manager_t *ui, coffee_app_controller_t *app)
{
...
void coffee_ui_manager_tick(coffee_ui_manager_t *ui, uint32_t elapsed_ms)
{
    ...
    if ((state == COFFEE_APP_COMPLETED && ui->app->state_elapsed_ms > COFFEE_COMPLETION_MS) ||
        (state == COFFEE_APP_CANCELLED && ui->app->state_elapsed_ms > COFFEE_CANCELLED_MS)) {
