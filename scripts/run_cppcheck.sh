...
readonly DEFAULT_BUILD_DIR="${PROJECT_ROOT}/build/linux-debug"
...
Run Cppcheck against the CMake compilation database.
...
main() {
    ...
    BUILD_DIR="$(make_absolute_path "${BUILD_DIR}")"
    REPORT_DIR="$(make_absolute_path "${REPORT_DIR}")"
    ...
    if ! run_command cppcheck \
        ...
        --file-filter="${PROJECT_ROOT}/config/*" \
    ...
    finding_count="$(extract_finding_count "${xml_report}")" || {
        error "Could not parse Cppcheck XML report."
        exit 1
    }
