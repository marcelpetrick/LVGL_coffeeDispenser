...
stage_coverage() {
    ...
    # Counter data written by a different compiler release makes gcov abort
    # with a version/stamp mismatch, so the run data of previous builds is
    # dropped. The counters are regenerated by the test run below anyway.
    if [[ -d "${coverage_dir}" ]]; then
        log "Removing stale gcov counter data."
