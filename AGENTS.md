...
`src/ui/perf_probe.c` hooks `LV_EVENT_RENDER_START/READY` and `LV_EVENT_FLUSH_START/FINISH`; in force-redraw mode it re-dirties the screen from `LV_EVENT_REFR_READY`, which is exactly one full-screen invalidation per rendered frame instead of the seven the main loop used to issue. Measured as pairs the two approaches render equally fast, so this is about not doing pointless work, not about speed - an earlier claim of 14% here came from an unpaired measurement and did not hold up. The percentile math is `src/platform/perf_stats.c` in `coffee_core` and is therefore unit-tested.

The report is named after the measured LVGL release (`LVGL_9.6.0_benchmark.md`), so keep one per release instead of replacing it, and pass `--pin-cpu` on any machine with frequency scaling or hybrid cores - unpinned runs scatter several times wider than the effects being measured. `docs/benchmark-lvgl-9.3-vs-9.6.html` compares two releases and is published through GitHub Pages from `master:/docs` (site root `docs/index.html`, so a new page there belongs in that index); regenerate both sides with the same probe whenever the harness changes, otherwise the comparison is not like for like.

**Never compare two sequential runs.** This machine moved the absolute render time by more than 10% between rounds - thermal throttling and background load - which is several times the effects being measured, and it produced a 1.23x result that repeated measurement put at 7%. Use `--compare <other build dir>`: it alternates the two builds run by run, so each pair meets the same clock state, and read the per-pair deltas rather than the absolute means. The report also carries `lv_mem_monitor()` and `ru_maxrss` figures, so a library bump can be judged on memory as well as on time. Measured FPS is capped by `LV_DEF_REFR_PERIOD` (33 ms), so the meaningful number is the per-frame render + flush cost.
